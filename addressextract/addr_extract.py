...
Date: 2020-09-04 16:30:03
'''
import pandas as pd
import sys, os
sys.path.insert(0, os.getcwd())

from addressextract import config
from addressextract.structures import AddrMap, Pca
from addressextract.structures import P, C, A
...
class AddressExtract(object):
    def __init__(self):
        self.pca_path = config.pca_path
        self.area_map, self.city_map, self.province_area_map, self.province_map, self.latlng = self._data_from_csv(self.pca_path)
        
    @staticmethod
    def _data_from_csv(pca_path) -> (AddrMap, AddrMap, AddrMap, dict, dict):
        # 区名及其简写 -> 相关pca元组
        area_map = AddrMap()
        # 城市名及其简写 -> 相关pca元组
        city_map = AddrMap()
    ...
    def is_munis(self, city_full_name):
        return city_full_name in munis

    def transform(self, location_strs, umap=myumap, index=[], cut=False, lookahead=8, pos_sensitive=False, open_warning=False):
        ...
        if not isinstance(location_strs, Iterable):
            from addressextract.exceptions import InputTypeNotSuportException
            raise InputTypeNotSuportException(
                'location_strs参数必须为可迭代的类型(比如list, Series等实现了__iter__方法的对象)')

        result = pd.DataFrame(
            [self._handle_one_record(addr, umap, cut, lookahead, pos_sensitive, open_warning) for addr in location_strs],
            index=index) \
    ...
    def _handle_one_record(self, addr, umap, cut, lookahead, pos_sensitive, open_warning):
        '''
        Descripttion: 处理单条记录
        param {type} 
        return {type} 
        ...
        if not isinstance(addr, str) or addr == '' or addr is None:
            ...
            return empty
        # 地名提取
        pca, left_addr = self._extract_addr(addr, cut, lookahead)
        # 填充市
        self._fill_city(pca, umap, open_warning)
        # 填充省
        self._fill_province(pca)

        result = pca.propertys_dict(pos_sensitive)
        result["地址"] = left_addr
        return result

    def _fill_city(self, pca, umap, open_warning):
        """填充市"""
        if not pca.city:
            # 从 区 映射
            if pca.area:
                ...
                if umap.get(pca.area):
                    ...
                    return
                if pca.area in self.area_map and self.area_map.is_unique_value(pca.area):
                    pca.city = self.area_map.get_value(pca.area, C)
                    return

            # 从 省,区 映射
            if pca.area and pca.province:
                newKey = (pca.province, pca.area)
                if newKey in self.province_area_map and self.province_area_map.is_unique_value(newKey):
                    pca.city = self.province_area_map.get_value(newKey, C)
                    return
    ...
    def _fill_province(self, pca):
        """填充省"""
        if (not pca.province) and pca.city and (pca.city in self.city_map):
            pca.province = self.city_map.get_value(pca.city, P)

    def _extract_addr(self, addr, cut, lookahead):
        """提取地址中的省,市,区名称
            Args:
                addr:原始地址字符串
                cut: 是否分词
        ...
        """
        return self._jieba_extract(addr) if cut else self._full_text_extract(addr, lookahead)

    def _jieba_extract(self, addr):
        """基于结巴分词进行提取"""
        import jieba
        result = Pca()
        pos = 0
        ...
        for word in jieba.cut(addr):
            ...
            if word in self.area_map:
                _set_pca('area', word, self.area_map.get_full_name(word))
            elif word in self.city_map:
                _set_pca('city', word, self.city_map.get_full_name(word))
            elif word in self.province_map:
                _set_pca('province', word, self.province_map[word])

            pos += len(word)

        return result, addr[truncate:]

    def _full_text_extract(self, addr, lookahead):
        """全文匹配进行提取"""
        result = Pca()
        truncate = 0
        ...
        while i < len(addr):
            ...
            for length in range(1, lookahead + 1):
                ...
                if word_next in filter_address_chars:
                    continue
                elif word in self.area_map:
                    defer_fun = _set_pca('area', i, word, self.area_map.get_full_name(word))
                    continue
                elif word in self.city_map:
                    defer_fun = _set_pca('city', i, word, self.city_map.get_full_name(word))
                    continue
                elif word in self.province_map:
                    defer_fun = _set_pca('province', i, word, self.province_map[word])
                    continue
...
if __name__ == '__main__':
    ...
    location_str=['我在鄱阳县中的隔壁万年县']
    df = extract.transform(location_str, cut=False, pos_sensitive=True)
    print(df)
